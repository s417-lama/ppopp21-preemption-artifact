...
void bench_hpgmg(mg_type *all_grids, int onLevel, double a, double b, double rtol){
     int     doTiming;
     int    minSolves[2] = {1, 10}; // do at least minSolves MGSolves
  ...
  for(doTiming=0;doTiming<=1;doTiming++){ // first pass warms up, second pass times
    ...
    /* if(doTiming==1){ */
    /*   if((minTime/timePerSolve)>minSolves[doTiming])minSolves[doTiming]=(minTime/timePerSolve); // if one needs to do more than minSolves to run for minTime, change minSolves */
    /* } */
    /* #endif */
    ...
    while( (numSolves<minSolves[doTiming]) ){
